...
La CSP mantiene `script-src-attr 'unsafe-inline'` por los handlers históricos y permite conexiones HTTPS para Firebase y proveedores configurables. Se bloquearon scripts inline y se corrigieron los puntos de escape revisados; falta reemplazar todos los handlers por eventos registrados en módulos para aplicar una CSP estricta también a atributos. En un despliegue público, configurar HTTPS, IP del cliente/proxy y almacenamiento compartido del limitador antes de usar varias instancias.
